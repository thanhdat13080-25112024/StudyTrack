...
def test_grade_status_consistency_rejected(client):
    ...
    assert (
        client.post(
            "/api/grades",
            json={"course_id": cid, "semester_id": sid, "status": "passed"},
            headers=h,
        ).status_code
        == 422
    )
    ...
    assert (
        client.post(
            "/api/grades",
            json={"course_id": cid, "semester_id": sid, "grade_10": 7.0, "status": "in_progress"},
            headers=h,
        ).status_code
        == 422
    )
...
def test_grade_filter_by_semester_and_null_grade(client):
    ...
    client.post(
        "/api/grades",
        json={"course_id": cid, "semester_id": s1, "grade_10": 9.0, "status": "passed"},
        headers=h,
    )
    cid2 = _course(client, h, "CS102")
    r = client.post(
        "/api/grades",
        json={"course_id": cid2, "semester_id": s2, "status": "in_progress"},
        headers=h,
    )
